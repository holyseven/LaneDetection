...
bool Segment2d::isNeighbor(Segment2d s){
	...
	bool b3 = s.indexZone[1] > indexZone[1] - 2 && s.indexZone[1] < indexZone[1] + 2;
	bool b4 = s.indexZone[3] > indexZone[3] - 2 && s.indexZone[3] < indexZone[3] + 2;
	if (b3 && b4)
		return true;
...
double Pair2d::weight(Pair2d* p2)
{
	...
	const double SCALE_ANG = ANG * ANG;
	//const double DIFDIRDIST = 10; // pixels.
	//const double SCALE_DIRDIST = DIFDIRDIST * DIFDIRDIST * 100;
	const double DIFCOLOR = 10;
	const double SCALE_DIFCOLOR = DIFCOLOR * DIFCOLOR * 16;
	const double DIFWIDTH = 2;
	const double SCALE_DIFWIDTH = DIFWIDTH * DIFWIDTH * 9;
	const double DIFALIDIST = 3;
	const double SCALE_DIFALIDIST = DIFALIDIST * DIFALIDIST;
	...
	double dif_align_dist = 0.5 * (dist_p2line(p2->_p1, this->_p12) + dist_p2line(p2->_p2, this->_p12));
	...
	return part_ang * part_align_dist * part_color * part_width;
}
...
­­­­		ou la différence de la pente de Sj et de Si est hors la tolérance
		...
		v est composé de deux bords, et on fait deux fois la translation pour obtenir les voisins de v.
		Comparer les couleurs de v et de ses voisins.
		Si le couleur de v n'est pas plus clair que celui de ses voisins
			continue;

		Si et Sj est considéré comme un couple (Pair2d).
	}
}
...
Pour chaque couple 'pair_i' dans pairs
{
	Pour tous les autres couples 'pair_j' dans pairs
	{
		...
		pair_j sera connecté par pair_i.
	}
...
void LaneDetection::AlgoFilterLanes(ntuple_list line_out)
{
	...
	for (int i = 0; i < n_size; i++)
	{
		...
		for (int j = 0; j < n_size; j++)
		{
			...
			pair2d.setMeanColor(mean_color[0]);
			pair2d.setMeanWidth( 0.5 * (dist_p2p(entire_v[0].p1, entire_v[1].p1) 
				+ dist_p2p(entire_v[0].p2, entire_v[1].p2)) );
	...
	for (int i = 0; i < n_pairs; i++)
	{
		...
		while (!stk.empty())
		{
			int p = stk[stk.size() - 1]; stk.pop_back();
			if (!drawn[p])
			{
				vec_i.push_back(p);
				drawn[p] = true;
		...
		//vector<int> vec_i_t;
		//for (int k = 0, m = 0; k < vec_i.size(); k++)
		//{
		//	int n = vec_i[k];
		//	double *ptr_row_weights = weights.ptr<double>(n);
		//	double mean_weight = 0;
		//	for (m = 0; m < vec_i.size(); m++)
		//	{
		//		if (m != k)
		//			mean_weight += ptr_row_weights[vec_i[m]];
		//	}
		//	mean_weight /= (vec_i.size() - 1);
	...
	//	for (int j = i+1; j < n_pairs; j++)
	//	{
	//		if (pair_i->aligned(&pairs[j]) && pair_i->parallele(&pairs[j]))
	//		{
	//			vec_int.push_back(j);
	//		}
	//	}
