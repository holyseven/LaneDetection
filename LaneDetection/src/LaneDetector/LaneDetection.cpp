...
LaneDetection::LaneDetection(const Mat &_ipmImage, const int x){
	_ipmImage.copyTo(ipmImage);
	_ipmImage.copyTo(processImage);
	center_x = x;
}

LaneDetection::LaneDetection(const Mat &_ipmImage, const Mat &_ipmMask, const int x){
	_ipmImage.copyTo(ipmImage);
	_ipmImage.copyTo(processImage);
...
void LaneDetection::run(int method, int nameIndex_showImage){
	if (method == 1)
		method1();
	if (method == 2)
		method2();
